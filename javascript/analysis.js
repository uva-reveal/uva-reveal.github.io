  ...
  // define parsed data
    ...
    Object.entries(data_parsed).map( (b) => {
      ...
      if (typeof(key) === "number" || typeof(items) === "number"){
       ...
       //this will be a link
  //         if (search_results > 0) {
  //       $('#results').append('<a href="https://search.lib.virginia.edu/catalog/u6731744">f' + '<br>');
...
// define parsed data
  ...
  // using the [key, value] pairs from the parsed entries
    ...
    if (typeof(key) === "number" || typeof(items) === "number"){
     ...
     if ((search_results) + (search_results2) > 1) {
     $('#results').append(data['response']['docs'][0]['title_display'] + '<br>');
}
     //this will be a link
...
$('#results').html('');
  ...
  // this is +5 to make the search results ONLY include things that are above 5, meaning that it has BOTH array 1 and array 2
       ...
       //this will be a link
        // $('#results').append(json_file);
        console.log("number of search hits: " + (search_results + search_results2));
  });
  }
